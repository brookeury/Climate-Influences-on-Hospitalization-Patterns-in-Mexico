...
# Group by 'year', 'month', and 'state', then calculate the mean of 'acpcp'
rain_df_avg_weekly_state <- rain_df %>%
  group_by(year, week, state) %>%
  summarise(acpcp_avg = mean(acpcp, na.rm = TRUE)) %>%
  ungroup()
```
...
temp_df_avg_weekly_state <- temp_df %>%
  group_by(year, week, state) %>%
  summarise(air_avg = mean(air, na.rm = TRUE)) %>%
  ungroup()
```
...
# Add a column for each range with 1 or 0 based on air_avg
for (i in seq_along(labels)) {
  ...
  df[[range_name]] <- ifelse(
    df$air_avg >= lower & df$air_avg < upper, 
    1, 
    0
  )
}
...
for (i in seq_along(labels)) {
  ...
  # Create a column for each range
  df[[range_name]] <- ifelse(
    df$acpcp_avg >= lower & df$acpcp_avg < upper, 
    1, 
    0
  )
}
